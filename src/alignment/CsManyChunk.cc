// $Id: CsManyChunk.cc,v 1.14 2008/06/05 13:15:00 rgazda Exp $
#include <string>
#include <vector>
#include <list>
#include <iostream>
...
int main( int argc, char *argv[] )
{
  ...
  //=== load Arguments
  if( argc < 3 ){
    cout << "Usage: ManyChunk [-r] [-q <queue>] <ExecProg> <optionFile> [<ChunkID1> <ChunkID2> <...>]" << endl;
    return 0;
  }
  ...
  while( iArg < (unsigned int) argc && argv[iArg][0] == '-') {
    ...
    else if( string( argv[iArg] ) == "-q" ) {
      iArg++;
      queue = string( argv[iArg] );
    }
    iArg++;
  }
  ...
  //  unsigned int iDF = 0; // unused variable (jj)
  string DataFile;   
  ...
  if( doReload ) {
    ...
    //    procbuf pb( Task.c_str(), ios::in );
    //    istream in( &pb );
    //istringstream in( Task.c_str() );
    FILE *pipe;
    pipe = popen(Task.c_str(), "r");
    boost::fdistream in(fileno(pipe));
  ...
  if( access( Dir.c_str(), R_OK ) != 0 ) {
    ...
    //    procbuf inBuf( Task.c_str(), ios::in );
    //istringstream inBuf( Task.c_str() );
    system(Task.c_str());
  ...
  for( unsigned int i=0; i< Chunks.size(); i++ ) {
    ...
    if( useDB ){
      ...
      newFile = SavePath+"/"+"cdr"+Chunks[i]+"-"+RunNumber+".raw";
      cout << "ManyChunk - \"" << newFile << "\" -> ";
    }
    
    int iStart = optFile.rfind("/");
    string optOutFile( Dir+"/"+ optFile.substr(iStart+1, optFile.length() - iStart+1) + "." + Chunks[i] );
    cout << "\"" << optOutFile << "\" - ";
  ...
  for( unsigned int i=0; i<OptFiles.size(); i++ ) {
    ...
    int iStart = optFile.rfind("/");
    string optFileShort = optFile.substr(iStart+1, optFile.length() - iStart+1);
    outBSubScript+= "."+ optFileShort + "."+ChunksOK[i];
    ...
    //=== Print BSub options
    string log = pwd + "/log/log." + optFileShort + "." + RunNumber + "." + ChunksOK[i];
    subout << "#BSUB -q " << queue << endl;
