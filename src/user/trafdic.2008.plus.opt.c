...
// $Id: trafdic.2008.plus.opt.c 13326 2012-03-23 17:57:53Z ybedfer $
...
// TraFDic on 2008 >0 hadron data (i.e. run# >= 71020) @ CERN.
#elif C2OPT_COMPUTERFARM == 2
// TraFDic on 2008 >0 hadron data (i.e. run# >= 71020) @ gridKa.
#elif C2OPT_COMPUTERFARM == 3
// TraFDic on 2008 >0 hadron data (i.e. run# >= 71020) @ Lyon.
#elif C2OPT_COMPUTERFARM == 4
// TraFDic on 2008 >0 hadron data (i.e. run# >= 71020) @ Compass ONLINE.
#else
// TraFDic on 2008 >0 hadron data (i.e. run# >= 71020).
#endif
...
// (Cf. also dedicated file for pi- data)
...
// of options infra, is enabled by default, since no run-by-run alignment is
// availble for a genuine fix, for the proton data.
#endif
...
//					   ***** INPUT DATA...
#  if   C2OPT_COMPUTERFARM == 1
Data file	/castor/cern.ch/compass/data/2008/raw/W39/cdr23014-71021.raw
#  elif C2OPT_COMPUTERFARM == 2
Data file 	/grid/fzk.de/compass/compass2/data/raw/W24/cdr13009-66806.raw
#  elif C2OPT_COMPUTERFARM == 3
Data file	cchpsscompass:/hpss/in2p3.fr/group/compass/data/2007/raw/W24/cdr13009-66806.raw
#  elif C2OPT_COMPUTERFARM >= 4
Data file	/castor/cern.ch/compass/data/2008/raw/W39/cdr23014-71021.raw
#  endif
#endif
...
// Reset the beam definition ("trafdic.2008.opt" corresponds to <0 hadron data taking).
TraF	iCut	[15]	1	// Beam charge.		(TEv::TracksFit2,GetIncidentTracks)

// ***** WORK-AROUND for RUN-TO-RUN MISALIGNMENT
// (To be disabled if RUN-BY-RUN alignment becomes avilable.)
...
TraF	DetNameOff	VO	VI	DW02X2	DW02Y1	HI05	// Turn off "DW02Y1" by default for proton data.
// ==> ==> Implications:
...
// - RUN BY RUN alignment, if provided for supra, allows to set =0 the artificial blowing-up of SI routes and uncertainties:
