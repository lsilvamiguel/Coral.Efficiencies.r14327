// $Id: CsDriftChamberDetector.h,v 1.59 2010/01/24 16:10:40 suhl Exp $

/*!
   \file    CsDriftChamberDetector.h
...
#ifndef CsDriftChamberDetector_h
#define CsDriftChamberDetector_h
...
class CsDriftChamberDetector : public CsDetector {
 public:
 
  /*! \fn CsDriftChamberDetector( const int row, 
  ...
  */
  CsDriftChamberDetector( const int    row,
  ...
  bool operator==( const CsDriftChamberDetector& ) const; //!< "equal to" operator
  bool operator<( const CsDriftChamberDetector& ) const; //!< "less than" operator
  ...
  */
  ...
  double getDistToWire( const double time, const int wire, bool &error );
 ...
 private:
  ...
  CsHist2D* H_ch_vs_r_;    //!< Channel versus r (full range)
  ...
  public:
    ...
    friend std::istream& operator>>(std::istream& in,CsDriftChamberDetector::CalibrationData &c);
  };

};

#endif // CsDriftChamberDetector_h
