// $Id: CsDWDetector.cc,v 1.15 2010/01/28 12:51:25 tnagel Exp $

/*!
   \file    CsDWDetector.cc
...
CsDWDetector::CsDWDetector( const int    row,
  ...
  
{
  ...
  if (status) {
	...
	if (*Is=="DW" || *Is=="all") decodeCard_ = true;
    }
...
#ifdef DEBUG_EFF
    cout<<"**** LS *** : CsDWDetector:: "<< mcEffMapsEnabled_<<endl;  
#endif
  }
  else CsErrLog::msg(elWarning,__FILE__,__LINE__,
                     "%s: eff. Map will NOT be used",TBname);

  //                                         ***** DW TREATED LIKE a MWPC? *****
  ...
  alwaysTwo_ = opt->getOpt("DW","make always two clusters");

  // ***** PROPAGATION TIME CORRECTION *****
  //  Not (yet?) accounted for in DW's (for assumed to be negligible compared to
  // DW drift times).
...
//____________________________________________________________________________
bool CsDWDetector::operator==( const CsDWDetector& det ) const {
...
// (Used in CsDWDetector::makeMCDecoding())
class DWdig 
{
public:
  DWdig(int w, double t, CsMCHit *r, int l) {
  ...
  int    lr;    // Left/right
  bool operator < (const DWdig &gd) const 
  { 
...
//____________________________________________________________________________
void CsDWDetector::makeMCDecoding() {
  ...
  if (!CsGeant3::Instance()->isAnNtFile()) {
    ...
    list<DWdig> ld; // ***** LIST OF TEMPORARY DIGITS *****
	...
	if ((((*Ih)->getMCTrack())->getParticle())->getCharge() ||
	  ...
	  ld.push_back(DWdig(wire,driftExact,*Ih,lr));
	}
      }
    }
	...
	if (((thit->getMCTrack())->getParticle())->getCharge() ||
	  ...
	  for (int i = wireF; i!=wireL+incr; i += incr) { 
...
#ifdef DEBUG_EFF
              cout<<"**** LS *** : CsDWDetector::makeMCDecoding : "<< GetTBName() <<" - "<<     
...
#ifdef DW_FIX_DZ_BUG
	    if (!inActiveArea(thit->getX(),thit->getY())) {
	      double drift; int dummyCounter = 0; do {
		drift = driftExact + spSli_ * tiSli_ * CsRandom::gauss();
	    ...
	    ld.push_back(DWdig(i,driftExact,*Ih,lr));
...
//____________________________________________________________________________
void CsDWDetector::DecodeChipDigit(const CS::Chip::Digit &digit)
{

  const CS::ChipF1::Digit *d = dynamic_cast<const CS::ChipF1::Digit *>(&digit);
  if( d==NULL ) throw CS::Exception("CsDWDetector::DecodeRawData(): Wrong digit type");

  int ch = d->GetChannel();
  if( ch < 0 || ch >= int(getNWir()))
  throw CS::Exception("CsDWDetector::DecodeRawData() ==> Unexpected wire number %d for SiFi %d %s",
  ...
  //#define Drift_W45_EXCLUDE_INNER
#ifdef Drift_W45_EXCLUDE_INNER
  if (GetTBName()[3]=='4' || GetTBName()[3]=='5') {
    if (GetTBName().c_str()[4]=='Y') {
      if (nWir_/2- 8<=ch && ch<nWir_/2+ 8) return;
    }
    else
      if (nWir_/2-12<=ch && ch<nWir_/2+12) return;
  }
#endif
...
//____________________________________________________________________________
double CsDWDetector::getDistToWire( const double t, bool &error ) {
...
//____________________________________________________________________________
double CsDWDetector::getDRDT( const double t, bool &error ) {
...
void CsDWDetector::clusterize() {
  ...
  else {                // ********** PROCESS DWs USING DRIFT TIME ********** 
    int nCorDigits = 0;
    list<CsDigit*>::iterator Id;
    list<CsDigit*> digits = getMyDigits();
...
//____________________________________________________________________________
double CsDWDetector::getCorrU(const CsCluster *c,
			      const double x, const double y, const double tt,
			      bool &error)
{
  // *** This is the implementation the CsDetector virtual method for DW's.
  // - It does not correct for propagation time (assumed to be negligible
  //  compared to DWs' drift time). But does correct for the T0 offset of
...
//____________________________________________________________________________
void CsDWDetector::updateClusters(double time)
{
  HepMatrix iRotM(3,3); iRotM = getRotWRSInv();
  double wireDCorr = wirD_ + iRotM(1,1) * _deltax + iRotM(1,2) *_deltay; 
...
//____________________________________________________________________________
bool CsDWDetector::_readRTRelation( void )
{
...
//=============================================
istream& operator>>(istream& in,CsDWDetector::CalibrationData &c) {
  in >> c.t0;     // read t0
...
void CsDWDetector::readCalibration(time_t timePoint){
  ...
  if ((getenv("COND_DB_DEBUG")!=0) || (getenv("COND_DB_DEBUG_DW")!=0)) {
    cout <<GetTBName()<<endl;
    cout << data << endl;
  }
  ...
  if( hasRT_ )
    ...
    delete calib;
    hasRT_ = false;
    cout << "CsDWDetector::readCalibration() ==> " << GetTBName() << " RT relation from database not valid\n";
  }

}

//LS Eff                                                                                               
void CsDWDetector::readMCEffMaps(time_t timePoint)                                          
{                                                                                                   
  ...
  while( getline(is,line) )                                                                         
    {                                                                                               
      ...
      if( 3!=sscanf(line.c_str(),"%d %g %g",&c,&mcEf,&mcEf_err) )                                   
        throw std::logic_error("CsDWDetector_MCEff::read(): bad line structure.");          
