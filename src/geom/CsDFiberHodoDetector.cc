// $Id: CsDFiberHodoDetector.cc,v 1.36 2010/09/21 01:50:50 ybedfer Exp $

/*!
   \file    CsDFiberHodoDetector.cc
   \brief   Compass Fiber Hodoscope like detector Class.
   \author  Benigno Gobbo
   \version $Revision: 1.36 $
   \date    $Date: 2010/09/21 01:50:50 $
*/
...
CsDFiberHodoDetector::CsDFiberHodoDetector( const int    row,
...
void CsDFiberHodoDetector::BookHistograms( void )
{
  ...
  int range_low = -22000;
  int range_high = -12000; // zoom & calibration histogram range
...
void CsDFiberHodoDetector::DecodeChipDigit(const CS::Chip::Digit &digit)
{
  const CS::ChipF1::Digit *d = dynamic_cast<const CS::ChipF1::Digit *>(&digit);
  if( d==NULL )
    throw CS::Exception("CsDFiberHodoDetector::DecodeRawData(): Wrong digit type");
  ...
  if( d->GetChannel() < 0 || d->GetChannel() >= int(getNWir()))
    throw CS::Exception("CsDFiberHodoDetector::DecodeRawData() ==> Unexpected wire number %d for SiFi %d %s",
...
//     if(calib_flag[d->GetChannel()]!=0 && calib_flag[d->GetChannel()]!=2)

    //!< Patch from Hugo to accept also unknown flag 8
    if(calib[d->GetChannel()].flag!=0 && calib[d->GetChannel()].flag!=2 && calib[d->GetChannel()].flag!=8)
    useIt=false;
...
// (Used in CsDFiberHodoDetector::clusterize())
//
...
void DFIclusterize(bool do_clustering, list<FIdig>& ld, list<FIcl>& lc, CsHist1D **mH, double cluDtMax_)
{
...
void CsDFiberHodoDetector::clusterize() {
  ...
  DFIclusterize(this->do_clustering, ld, lc, mH1, cluDtMax_); // do FI clusterizetion <==========
...
bool CsDFiberHodoDetector::operator==( const CsDFiberHodoDetector& det ) const {
...
void CsDFiberHodoDetector::makeMCDecoding() {
...
void CsDFiberHodoDetector::readCalibration(time_t timePoint) {
  ...
  if(calib.size()!=0){
	...
	cout<<"CsDFiberDetector::readCalibration() invalid channel number "
	    <<chn<<" in "<<GetTBName()<<endl;
    }
    string debug_level;
    if ((getenv("COND_DB_DEBUG")!=0) || (getenv("COND_DB_DEBUG_DFIBER")!=0)) {
      cout << GetTBName() << endl;
      for (it=calib.begin(); it!=calib.end(); it++) {
        cout << (*it).chn << " ";
        cout << (*it).data << " ";
...
istream& operator>>(istream& in, CsDFiberHodoDetector::Calib &c) {
  string line;
  std::getline(in, line);
...
istream& operator>>(istream& in, vector<CsDFiberHodoDetector::Calib> &vc) {
  CsDFiberHodoDetector::Calib c;

//  vc.clear();
  while (in >> c) {
    if (((int)vc.size()) < (c.chn+1)) vc.resize(c.chn+1, CsDFiberHodoDetector::Calib(0, 0.0, 17));
    vc[c.chn] = c;
  }
