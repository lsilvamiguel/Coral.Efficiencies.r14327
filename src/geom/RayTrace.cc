...
int printxy=0;
int verbose=0;
// set to 1 to suppress most error messages of subroutines...
// set to 2 to suppress messages from the main tracing routine.
int quiet=2;
...
int OptSyst::RayTrace(LightRay &photon)
{
  ...
  if(photon.getval(6)<=0){
    cerr << "OptSyst::RayTrace: Photon direction not valid!" << endl;
    return -1;
  }
  ...
  if(e>0.) e=hc/e;
    ...
    {
      cerr << "OptSyst::RayTrace: Photon energy not valid!" << endl;
      return -1;
    }
  ...
  for (int k=0 ; k<7 ; k++)
   ...
   if(debug)
	{      
      ...
      if(status) 
	{
	  if(quiet<2) cerr << "OptSyst::RayTrace: Error in Refract at surface "<< k+1 << ", aborting function RayTrace! (status == " << status << ")"<< endl;
...
//  if(printxy) cout << ipx << "\t" << ipy << endl;  
  ...
  {
    if(!quiet)cerr << "Unexpected pins ipx = " << ipx <<" ipy = " << ipy << endl;
    if(!quiet)cerr << " xout = " << xout << " yout = " << yout << endl;
    return ipin;
   }
  ipin = ipx + 4*ipy;
...
int CoordinateBreak::Refract(LightRay &photon)
{
  ...
  if(status) 
    {
      if(!quiet)cerr << "Error in TransformRay, aborting function Refract! (status == " << status << ")"<< endl;
      return status;
    }
  return status;
}
...
int OpticalPlane::Refract(LightRay &photon)
{
  ...
  if(status) 
    {
      if(!quiet)cerr << "Ray will be vignetted, aborting function Refract! (status == " << status << ")"<< endl;
      return status;
    }
  ...
  if(status) 
    {
      if(!quiet)cerr << "Error in FindIntersection, aborting function Refract! (status == " << status << ")"<< endl;
      return status;
    }
  ...
  if(status) 
    {
      if(!quiet)cerr << "Error in FindNormal, aborting function Refract! (status == " << status << ")"<< endl;
      return status;
    }
  ...
  if(status) 
    {
      if(!quiet)cerr << "Error in DoRefraction, aborting function Refract! (status == " << status << ")"<< endl;
      return status;
    }
  ...
  if(status) 
    {
      if(!quiet)cerr << "Error in TransformRayBack, aborting function Refract! (status == " << status << ")"<< endl;
      return status;
    }
...
// all lengths are in cm!
{
  ...
  const double topend=100000.; // borders of the CsI cathodes in MRS and cm
  const double sideend=100000.;
  const double distic=0.95;  // z-distance in mm of lens center to CsI cathode
  ...
  static CoordinateBreak FirstLensLoc[4][12][12];
  for(int k=-6; k<6; k++){
    for(int i=-6 ; i<6 ; i++){
      ...
      FirstLensLoc[0][5-k][5-i].SetLoc(auxdist, -25.09 - k*48.2 , -25.56 - i*46.7 );  // top jura
      FirstLensLoc[1][k+6][5-i].SetLoc(auxdist,  25.09 + k*48.2 , -25.56 - i*46.7 );  // top saleve
      FirstLensLoc[2][5-k][i+6].SetLoc(auxdist, -25.09 - k*48.2 ,  25.56 + i*46.7 );  // bottom jura
      FirstLensLoc[3][k+6][i+6].SetLoc(auxdist,  25.09 + k*48.2 ,  25.56 + i*46.7 );  // bottom saleve
    }
  }
...
// to play around...
  ...
  for(int k=ilx-1 ; k<ilx+2 ; k++){
	...
	FirstLensLoc[bottom+saleve][k][i].Refract(testray);
...
int OptSyst::TransPhotonDir(double xr, double yr, double zr, double xm, double ym, double zm, double xd, double yd, double e, int dn, LightRay &Photon)
// all lengths are in cm!
{
  ...
  if((!(top+bottom) || !(jura+saleve)) || (dn < 17)){
    cerr << "OptSyst::TransPhotonDir: Photon not on one of the four central cathodes!" << endl;
    if(verbose){      
      cout << "---------------------------------------------------------------------------------------" << endl;
      cout << "OptSyst::TransPhotonDir: Photon not on one of the central cathodes: " << cathode << endl;
      cout << "---------------------------------------------------------------------------------------" << endl;
    }
    return cathode;
  }

  cathode=4+bottom*7+saleve*2;
...
// to play around...
  ...
  if(e>0.) Photon.SetWave(hc/e);
    ...
    {
      cerr << "OptSyst::TransPhotonDir: Energy must be > 0!" << endl;
      return(-1);
    }
  ...
  // photon direction in MRS (variables renamed to correspond to DRS)
  d[2] = xm-xr;  
  d[0] = ym-yr;
  d[1] = zm-zr;
  ...
  Photon.SetDir(d);
  if(debug) {
    cout << endl << "Unnormalised direction of photon (variables renamed):" << endl;
    Photon.print();
  }
  Photon.norm();
  if(debug) {
    cout << endl << "Normalised direction of photon:" << endl;
    Photon.print();
  }



  // Here, the photon direction will be transformed
  // to the DRS (= CsI frame) 
  if(top){
    CsIupper.Refract(Photon);
    if(debug) cout << endl << "Transforming to top cathode" << endl;
  ...
  if(debug) {
    cout << "Origin in MRS, direction in CsI RS:" << endl;
    Photon.print();
  }
  ...
  s[0] = xd*10.;  // convert to mm
  s[1] = yd*10.;
  s[2] = 0; // correct z coordinate is determined by the ray tracing routine
  // transform to ZEMX frame
  s[0]=s[0]*bottom-s[0]*top;
  s[1]=s[1]*top-s[1]*bottom;
  ...
  if(verbose){
    cout << "Photon parameters (in ZEMAX):  \t";
    Photon.print();
  }
...
int OptSyst::CheckEff(LightRay photon,double scale_factor)
{
  double wav=photon.getval(7);  
  double eff, effarr[2][51];

//  cout << "OptSyst::CheckEff" << endl;

  if(wav <= 200 || wav >= 700) return 1;

// mean PMT efficiencies of SM0064 SM0081 SM0113
// between 200 and 700 nm as provided by Hamamatsu,
// the maximum eff (25.8% at 360 nm) has been scaled to 1 
  effarr[1][0] = 0.219226;
  effarr[1][1] = 0.254452;
  effarr[1][2] = 0.292775;
  effarr[1][3] = 0.33742;
  ...
  effarr[1][8] = 0.683614;
  effarr[1][9] = 0.76284;
  effarr[1][10] = 0.840001;
  effarr[1][11] = 0.885808;
  effarr[1][12] = 0.926582;
  effarr[1][13] = 0.956904;
  effarr[1][14] = 0.971485;
  effarr[1][15] = 0.99613;
  effarr[1][16] = 1;
  effarr[1][17] = 0.998195;
  effarr[1][18] = 0.99755;
  effarr[1][19] = 0.984904;
  effarr[1][20] = 0.978969;
  effarr[1][21] = 0.968646;
  effarr[1][22] = 0.957033;
  effarr[1][23] = 0.92813;
  effarr[1][24] = 0.903356;
  effarr[1][25] = 0.870711;
  effarr[1][26] = 0.827485;
  effarr[1][27] = 0.771098;
  effarr[1][28] = 0.726453;
  effarr[1][29] = 0.681807;
  effarr[1][30] = 0.649162;
  effarr[1][31] = 0.592259;
  effarr[1][32] = 0.490839;
  effarr[1][33] = 0.382323;
  effarr[1][34] = 0.314452;
  effarr[1][35] = 0.271484;
  effarr[1][36] = 0.237033;
  effarr[1][37] = 0.203484;
  effarr[1][38] = 0.172645;
  effarr[1][39] = 0.139613;
  effarr[1][40] = 0.110968;
  effarr[1][41] = 0.0841291;
  effarr[1][42] = 0.0606452;
  effarr[1][43] = 0.0401291;
  effarr[1][44] = 0.0259355;
  effarr[1][45] = 0.016;
  effarr[1][46] = 0.00929033;
  effarr[1][47] = 0.00490323;
  effarr[1][48] = 0.00258065;
  effarr[1][49] = 0.00141936;
  effarr[1][50] = 0.000645162;

  for(int k = 0 ; k < 51 ; k++) effarr[0][k] = k*10. + 200.;

  eff = -1.;
  int k=0;

  while(wav > effarr[0][k]) k++;

  eff = (wav - effarr[0][k-1])*((effarr[1][k] - effarr[1][k-1])/(effarr[0][k] - effarr[0][k-1])) + effarr[1][k-1];

  eff = eff*scale_factor;
//  cout << wav << " nm: eff = " << eff << endl;

  double detval = (random() % 10001) /10000. ;
//  cout << detval << endl;
  if(detval > eff){
//     cout << "Photon nicht nachgewiesen: wav = " << wav <<  "  eff = " << eff << "  detval = " << detval << endl;
     return 1;
  }
  return 0;
}
