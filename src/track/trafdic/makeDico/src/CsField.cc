...
// $Id: CsField.cc,v 1.17 2010/06/26 01:07:58 ybedfer Exp $

/*!
   \file CsField.cc
   \brief Compass Magnetic Field Class specific for Dico: measured field is
   extended (smoothly) beyond map boundaries (in "getFieldSM1").
   \author  Y.B.
   \version $Revision: 1.17 $
   \date    $Date: 2010/06/26 01:07:58 $
*/
...
void CsMagInfo::readPolarization(CsField *Field, int Run)
{
  ...
  if (CsOpt::Instance()->getOpt("CsMagInfo","MySQLDB")) {
    ...
    //    "~ybedfer/coral.utils/magInfo.pl"
	...
	//  soleno dipole up         +/-    down        +/-    target# nmr   nmr#
	in>>solCur>>dummy>>pol.first>>dummy>>pol.second>>dummy>>dummy>>dummy>>dummy;
...
void CsMagInfo::scaleSM2withNMR(CsField* Field, int Run)
{
  ...
  if (CsOpt::Instance()->getOpt("CsMagInfo","MySQLDB")) { // ...from MySQL *****
	...
	// One gets successively:
	//  soleno dipole up     +/-    down   +/-    target# nmr   nmr#
	in>>dummy>>dummy>>dummy>>dummy>>dummy>>dummy>>dummy>>nmr>>dummy;
	if      (Run==0)    // What's run==0? Don't know!? No documentation...
    ...
    in.close();
    if (sm2nmr==0) CsErrLog::msg(elBasicInfo,__FILE__,__LINE__,
        "SM2 NMR not found for run #%d in file \"%s\" => No SM2 rescaling.",
				 Run,path.c_str());
  }

  if (sm2nmr>0) {
    if (rescaling!=1) sm2nmr *= rescaling;
    ...
    float field_x, sm2map, field_z;
    Field->getFieldSm2(x,y,z,field_x,sm2map,field_z);
    sm2map = fabs(sm2map); // Absolute value! I assume DB NMR systematically >0.

    if (fabs(sm2nmr-sm2map)>1e-4) {  // ***** UPDATE FIELD SCALING FACTOR *****
      if (rescaling!=1) CsErrLog::msg(elBasicInfo,__FILE__,__LINE__,
"SM2: NMR(=%.4f) * Option-supplied scale(=%.4f) / map(=%.4f) =>  Rescaling=%.4f while detectors.dat scale =%.4f",
	sm2nmr/rescaling,rescaling,sm2map/fsc,fsc*sm2nmr/sm2map,fsc);
      else CsErrLog::msg(elBasicInfo,__FILE__,__LINE__,
"SM2: NMR(=%.4f) / map(=%.4f) => Rescaling=%.4f  while detectors.dat scale =%.4f",
    ...
    else if (rescaling!=1) CsErrLog::msg(elBasicInfo,__FILE__,__LINE__,
"SM2: NMR(=%.4f) * Option-supplied scale(=%.4f) / map(=%.4f) =>  Rescaling=%.4f while detectors.dat scale =%.4f",
	sm2nmr/rescaling,rescaling,sm2map/fsc,fsc*sm2nmr/sm2map,fsc);
    else CsErrLog::msg(elBasicInfo,__FILE__,__LINE__,
"SM2: NMR(=%.4f) / map(=%.4f) => Rescaling=%.4f  while detectors.dat scale =%.4f",
...
bool CsField::getFieldDipOx( float pos_x, float pos_y, float pos_z,
  ...
  if( fabs(pos_x) < xoxd_[NX_OXD-1] &&
    ...
    ii=0;
    while ( (fabs(pos_x)>xoxd_[ii] || fabs(fabs(pos_x)-xoxd_[ii])<1e-3) && ii<NX_OXD ) ii++;
    ii=ii-1;
    
    jj=0;
    while ( (fabs(pos_y)>yoxd_[jj] || fabs(fabs(pos_y)-yoxd_[jj])<1e-3) && jj<NY_OXD ) jj++;
    jj=jj-1;
    
    kk=0;
    while ( (fabs(pos_z)>zoxd_[kk] || fabs(fabs(pos_z)-zoxd_[kk])<1e-3) && kk<NZ_OXD ) kk++;
    kk=kk-1;
