...
CsECAL2::CsECAL2(const string &name, const string &geom_file)
...
// Check/Init fortran block
  if (options.fortran_reconstruction )
  {
    if( XYRegularGrid() && GetNColumns() == 64 && GetNRows() == 48 )
    {
// check the hole position/size
      int icell = GetCellOfColumnRow(34,23);
      if( icell >= 0 ) CsErrLog::mes( elFatal, " Fortran reco requested for CsECAL2:: Bad geometry(hole position) detected ! " );
      icell = GetCellOfColumnRow(35,23);
      if( icell >= 0 ) CsErrLog::mes( elFatal, " Fortran reco requested for CsECAL2:: Bad geometry(hole position) detected ! " );
      icell = GetCellOfColumnRow(34,24);
      if( icell >= 0 ) CsErrLog::mes( elFatal, " Fortran reco requested for CsECAL2:: Bad geometry(hole position) detected ! " );
      icell = GetCellOfColumnRow(35,24);
      if( icell >= 0 ) CsErrLog::mes( elFatal, " Fortran reco requested for CsECAL2:: Bad geometry(hole position) detected ! " );
    }
    else
    {
      CsErrLog::mes( elFatal, " Fortran reco requested for CsECAL2:: Bad geometry detected ! " );
    }
  }
...
void CsECAL2::SetDefaultSADCDigitizationParameters( void )
{
...
void CsECAL2::FillHistoLED4SADC(void)
{
//  bool debug = true;
 bool debug = false;
 bool ok=false;
  if( debug ) cout << " CsECAL2::FillHistoLED4SADC options.fill_histos = " << options.fill_histos << endl;
  ...
  if( first_time )
  {
    if( debug ) cout << " CsECAL2::FillHistoLED4SADC BOOKING " << endl;
    first_time = false;
    char dir_name[111];
    ok=(gDirectory->cd("/"));
    assert(ok);
...
//      if( !sadc_readout_[i] ) continue;
    ...
    if( debug ) cout << " CsECAL2::FillHistoLED4SADC BOOKING  OK " << endl;
  }

  if( debug ) cout << " CsECAL2::FillHistoLED4SADC Start filling sadc_samples_.size() = " << sadc_samples_.size() << endl;
...
//  look for corresponding time
  ...
  if( debug ) cout << " CsECAL2::FillHistoLED4SADC  OK " << endl;

  dir_save->cd();
  if( debug ) cout << " CsECAL2::FillHistoLED4SADC relly OK " << endl;
}

////////////////////////////////////////////////////////////////////////////////

void CsECAL2::CalibrateAtElectronBeam(void)
{
//   bool debug = true;
  bool debug = false;
//  bool only_one_particle = false;
  bool only_one_particle = true;
  if( debug ) cout << " CsECAL2::CalibrateAtElectronBeam Try to calibrate at electron beam " << endl;
...
void CsECAL2::CalibrateAtElectronBeamSpecial(void)
{
//  bool debug = true;
  bool debug = false;
  if( debug ) cout << " CsECAL2::CalibrateAtElectronBeamSpecial Try to calibrate at electron beam " << endl;
// Disable nparticle=1  if(reco_particles.size()!=1) return;
  int ngam = 0;
  for (size_t i=0; i<reco_particles.size(); i++)
    if( reco_particles[i].GetE() > options.particle_energy_threshold ) ngam++;
...
// disty selection
  ...
  if( !options.fill_histos ) return;
  if( debug )
    cout << " CsECAL2::CalibrateAtElectronBeamSpecial Try to Book/Fill calibration histos at electron beam " << endl;
...
void   CsECAL2::DrawCellHisto          (int icell)
{
...
bool   CsECAL2::GoodZone4Electrons          (double x, double y, double e)  const
{
...
void CsECAL2::EndOfJob( void )
{
  Reco::Calorimeter::EndOfJob();
  cout << " CsECAL2 now we need to write SADC profiles, wait a bit .. " << endl;
...
int  CsECAL2::StoreSADCDigit              (const int& icell, const CS::ChipSADC::Digit& digit, const bool& isLed) {
  if( CsCalorimeter::StoreSADCDigit(icell, digit, isLed) ) {
    ...
    if( !skip_cs_time_corrections_ )
    {
      ...
      {
        cerr << " CsECAL2::StoreSADCDigit CsTimingInfo->GetEC02Shift exit4debug  Exception:\n" << e.what() << endl;
        exit(1);
      }
      catch( ... )
      {
        cerr << " From CsECAL2::StoreSADCDigit CsTimingInfo->GetEC02Shift exit4debug Unknown exception: " << endl;
        exit(1);
      }
    }
