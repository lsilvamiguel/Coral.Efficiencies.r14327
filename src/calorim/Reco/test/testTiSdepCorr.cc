...
void usage(const char* exe) {
    ...
    std::cout << "        " << exe << " [file, e.g. test/testTiSdepCorr.dat] [histogram file, e.g. test/testTiSdepCorr.root]" << std::endl;
}
...
int main(int argc, char* argv[]) {
    ...
    calo->InputTiSdepCorr(calibS.str());

    const unsigned int cellsCount = 11;
    const unsigned int cellsId[cellsCount] = {  0,  1,  2,  3,
                                               15, 16, 17, 18, 19, 20,
                                               30 };
    ...
    const unsigned int steps = 500;
    for (unsigned int i=0; i<steps; ++i) {
        const double time = minT + i*(maxT-minT)/steps;

        for (unsigned int cell=0; cell<cellsCount; ++cell)
            g[cell]->SetPoint(g[cell]->GetN(), time, calo->SignalToEnergy(cellsId[cell], 1., time));
    }
