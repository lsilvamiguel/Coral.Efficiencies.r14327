/*
   $Source: /afs/cern.ch/user/g/gobbo/w0/coral/src/calorim/Reco/test/generate.c,v $ 
...
int main( int argc, char *argv[] )
{
  if( argc>4 )
  {
    cerr << "Usage: rec [Nevents] [r_noise] [r_pedestals]\n";
    return 1;
  }
  ...
  if( N_events_cal>0 )
  {
    ...
    for( size_t i=0; i<N_events_cal; i++ )
    {
      ...
      for( size_t j=0; j<N_cells; j++ )
      {
        ...
        calibration_events.back().Add( CellDataR(cells_noise_gen[j].mean+r1*cells_noise_gen[j].sigma,j) );
      }
    }
    /////////////////////////////////////////////////////////
