// $Id: Obj3D.cc,v 1.4 2010/02/03 18:22:23 suhl Exp $
/*!
   \file    Obj3D.cc
...
ostream &operator << (ostream &out,const Obj3D &o) 
{
  ...
  switch( o.shape_ ) {
    case Obj3D::pave:
	snprintf(text, 200, "%s pave     du=%13.4f, dv=%13.4f, dz=%13.4f", o.TBName_.c_str(), po->du_, po->dv_, po->dz_ );
	out<<text;
	break;
    case Obj3D::tunnel:
	snprintf(text, 200, "%s tunnel   duIn=%13.4f, dvIn=%13.4f, duOut=%13.4f, dvOut=%13.4f, dz=%13.4f", 
		o.TBName_.c_str(), 
		to->duIn_,  to->dvIn_, 
		to->duOut_, to->dvOut_, 
		to->dz_ );
	...
	break;
    case Obj3D::circle:
	snprintf(text, 200, "%s circle   r =%13.4f, dz=%13.4f",            o.TBName_.c_str(), co->r_,  co->dz_ );
	out<<text;
	break;
    case Obj3D::polyline:
	snprintf(text, 200, "%s polyline %zu", o.TBName_.c_str(), lo->points_.size() );
	out<<text;
	break;
