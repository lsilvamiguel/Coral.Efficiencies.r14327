// $Id: DetFileManager.cc,v 1.3 2007/06/01 09:13:51 conrad Exp $
/*!
...
DetFileManager::DetFileManager( const char* detFileName ): 
  TObject()
{
  ...
  ifstream in( detFileName_.c_str(), ios::in );
  if( !in ) {
    cout << "DetFileManager::DetFileManager - ERROR: cannot read file \"" 
         << detFileName_  << "\"." 
         << endl;
  ...
  unsigned int lineNumber = 0;
    ...
    // detector info
      ...
      else if( di->TBName_.substr(0,2) == "MB" ) {
        string TBTmp = di->TBName_.substr(0,7);
        tmp = _GetDetSelection( TBTmp );
      }  
      ...
      default:
        cout<<"DetFileManager::DetFileManager - FATAL - line "<<lineNumber<<" several dets with TBName \""<<tmp.front()->TBName_<<"\" saved independant."<<endl;
        exit( 0 );
      }
    ...
    // dead zone info
      ...
      switch( tmp.size() ) {
        case 0: 
        cout<<"DetFileManager::DetFileManager - FATAL - line "<<lineNumber<<" no detector associated to deadZone \""<<dead->TBName_<<"\"."<<endl;
        exit( 0 );
        break;
        ...
        default:
        cout<<"DetFileManager::DetFileManager - FATAL - line "<<lineNumber<<" several independant detectors associated to deadZone \""<<dead->TBName_<<"\"."<<endl;
        exit( 0 );
      }
    }       
  }
...
bool DetFileManager::MatchCenters( const char* det0, const char* det1 )
{
  ...
  for( map<DetectorInfo*,DetectorInfo*>::iterator I = sel.begin(); I != sel.end(); I++ ) {
    ...
    A = Utils::InvertMatrix( A );
    B = TMatrix( A, TMatrix::kMult, B );
  
    // calculate x, y displacements
    double dx0 = di0_M->irotM_(1,0)*B(0,0);
    double dy0 = di0_M->irotM_(1,1)*B(0,0);
    double dx1 = di1_M->irotM_(1,0)*B(1,0);
    double dy1 = di1_M->irotM_(1,1)*B(1,0);
...
//______________________________________________________________________________
bool DetFileManager::MoveDeadZone( const char* detSelection, double xOffset, double yOffset )
{ 
  vector< DetectorInfo* > dets = _GetDetSelection( detSelection );
