// $Id: DetectorInfo.cc,v 1.3 2010/03/16 08:33:04 suhl Exp $
...

{
  ...
  // update RT relation objects name
  rtInfo_->TBName_ = TBName_;
...
DetectorInfo* DetectorInfo::GetMain( void )
{
  ...
  else if( sub_[0]->TBName_.substr(0,2) == "MB" ) {
    for( unsigned int i=0; i< sub_.size(); i++ ) 
    if( sub_[i]->TBName_[7] == 'b' ||
        sub_[i]->TBName_[7] == 'r' ) {
      out = sub_[i];
...
string DetectorInfo::Dump( void )
{
  ...
  string out("");
  sprintf( line,
    " det %4i  %s  %s %2i %4i %8.2f %7.3f %7.3f %8.3f %11.4f %10.4f %10.4f ",
  ...
  sprintf( line,
    "%4i %11.4f %8.3f %5i %10.6f %5.3f %5.1f %6.1f ",
    rotMNum_,
...
ostream &operator << (ostream &out,const DetectorInfo &di) 
{
  ...
  snprintf(text, 200, "%13.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %10.4f %4d %s\n",
    di.zcm_,
    di.res_,
    di.cosTheta_,
    di.sinTheta_,
