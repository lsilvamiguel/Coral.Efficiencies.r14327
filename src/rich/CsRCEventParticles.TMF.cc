...
//--- Paolo  -  November 2000;  Rev. November 2001
...
//          --------------------------
        ...
        if( key->MCarloEvent() ) {
          if( key->myFileType() == 0 ) {
            lParticles_.push_back( new CsRCParticle( kPart,
...
//          -----------------------------------------
	    //cout << vDcInPart << endl;
          ...
          if( key->myFileType() >= 1 ) {
            lParticles_.push_back( new CsRCParticle( kPart, NULL,
		  	           vPoInPart, vDcInPart, vPoExPart,
				   vPosEmP, vDirEmP, vPosExW, vDirExW,
				   momPart, charge ) );
	  }
...
/*	bool entwMC = false;
	...
	if( nHits >= 50 && ihMin != 0 && zDiffMin < 1000. ) {
//------- project forward to the RICH entrance window
          double xDetUS = (*ihMin)->getX();
          double yDetUS = (*ihMin)->getY();
          double zDetUS = (*ihMin)->getZ();
...
//------- resolution monitoring histo.s
	  if( monHist ) {
            list<CsMCHit*>::iterator ihXMin = 0;
            double zXDiff = 0.;
...
//------- particle backward projection to point at RICH exit window
	  ...
	  int nCo = detNamePo.size();
	  double cClu[nCo], zClu[nCo];
	  double cosDet[nCo], sinDet[nCo];
	  int kClu = 0;
	  for( ic=lClusters.begin(); ic!=lClusters.end(); ic++ ) {
	    ...
	    for( unsigned int kDet=0; kDet<detNamePo.size(); kDet++ ) {
	      ...
	      //continue;
	      cosDet[kClu] = detes.front()->getCosAng();
	      sinDet[kClu] = detes.front()->getSinAng();
	      //cout << detName << "  " << unit << "  "
	      //     << cosDet[kClu] << "  " << sinDet[kClu] << "  " << endl;
	      cClu[kClu] = (*ic)->getU();
	      zClu[kClu] = (*ic)->getW() - zExitWind;
	      kClu++;
	      if( kClu >= nCo ) break;
	    }
	  }
...
//--- Paolo  -  November 2000
	...
	//std::cout << "tkTime =  " << tkMeanTime << std::endl;
...
//@@--------------------------------------------------------
	...
	if( !(zHelix0 > 0. && zHelix1 > 3500.) ) continue;
        //if( !(zHelix1 > 3500.) ) continue;
        //if( !(zHelix1 < 7000.) ) continue;
        //if( !(zHelix1 < 18000.) ) continue;
//@@---------------------------------------------------------
	int SM12 = -1;
        ...
        if( cop != 0. ) momPart = 1./fabs( cop );
        if( momPart > 300. ) continue;
        if( momPart > 170. ) continue;
        if( momPart < momMinAcc ) continue;
//@@--------------------------------------
//cout << "***  " << zHelix0 << "  " << zHelix1 << "  " << momPart << endl;
//std::cout << "tkTime =  " << tkMeanTime << "  " << momPart << std::endl;

	xh = tkMeanTime;
	yh = momPart;
	if( hist.hRC1220 ) hist.hRC1220->Fill( xh, yh );
//                         ----------------------------
...
//----- particle helix extrapolation to PT
	...
	double xPTg = 0.;
	double yPTg = 0.;
	if( vHelix[0].Extrapolate( -350., hxPTg ) ) {
	  xPTg = hxPTg.getX();
	  yPTg = hxPTg.getY();
...
//      helix extrapolation (use mag.field)
	...
	if( vHelix[kex].Extrapolate( zEntrWind, hxEntWin ) ) {
          ...
          //if( fabs( vPoInPart.x() ) >  1000. ) continue;
          //if( fabs( vPoInPart.y() ) >  1000. ) continue;
//------- entrance window size - as from Silvia  (021107)
          if( fabs( vPoInPart.x() ) >  1660. ) continue;
          if( fabs( vPoInPart.y() ) >  1205. ) continue;
...
//    rev.      March 2003
      ...
      fh = rfio_open( fileName, O_RDONLY, 0 );
//-------------------------------------------
      if( fh < 0 ) {
	cout << endl;
...
//--------------------------------------
      if( iret == -1 ) {
        cout << " RICHONE, CsRCEventParticles::setMyParticle() : ";
...
//  ------------------------
	...
	//cout << vRunNo[kRun] << "  " << vRunIndex[kRun] << endl;
    }
    ...
    for( size_t kRun=0; kRun<vRunNo.size(); kRun++ ) {
      ...
      if( pos < 0  ||  pos > sFileName.size() ) continue;
